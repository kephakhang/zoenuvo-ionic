...
export class AuthServiceProvider {
  ...
  constructor(
    ...
    public themeService: ColorModeService
  ) {
    this.init();
  ...
  public async get2(url: string, params?: HttpParams, isLoading?: boolean) {
    return new Promise(async (resolve, reject) => {
      const headers = new HttpHeaders({
        Accept: 'application/json;charset=UTF-8',
        'Content-Type': 'application/x-www-form-urlencoded',
      ...
      if (isLoading) {
        ...
        this.ahttp.get(url, options).subscribe(
          (res: any) => {
            loading.dismiss();
            resolve(res);
          },
    ...
    return new Promise((resolve, reject) => {
      this.getStorage(Common.USER).then(
        (user) => {
          if (user && user.jwt) {
            this.user = user;
            //const params = {'salt': this.getTimeNow()}
