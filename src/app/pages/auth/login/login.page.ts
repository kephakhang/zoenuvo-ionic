...
export class LoginPage implements OnInit {
  ...
  constructor(
    /* private snsAuth: SocialAuthService, */ public auth: AuthServiceProvider,
  ...
  ngOnInit(): void {
    if (environment.emailIdOnly) {
      ...
      this.idShouldBeReal = this.auth.message.get(
        'auth.login.emailShouldBeReal'
      );
    ...
    this.auth.getSession().then((user) => {
      this.auth.setStorage(Common.USER, user).then((u) => {
        ...
        this.auth.getStorage(Common.URI).then(
          (uri) => {
            if (uri === '/') {
              this.auth.goHome();
            } else {
  ...
  public isNotValidId(): boolean {
    const isValid = environment.emailIdOnly
      ? !this.isNotValidEmail()
      : !this.isNotValidEmail() || !this.isNotValidMobile();
    // if (isValid) {
    ...
    this.auth.login(this.user).then(
      (user: any) => {
        ...
        if (user) {
          ...
          this.submitted = false;
          this.showMessages.error = this.auth.message.get(
            'auth.login.unknownError'
          );
        }
      },
      (err) => {
        ...
        this.showMessages.error =
          err instanceof String
            ? (err as string)
            : this.auth.message.get('auth.register.unknownError');
      }
    );
  }
  /*
