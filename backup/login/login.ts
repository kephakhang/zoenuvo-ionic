...
export class LoginPage {
  ...
  ionViewWillEnter() {
    ...
    this.auth.getStorage(common.TERMS).then((value) => {
      if (value != null && value) {
        this.auth.removeStorage(common.TERMS);
  ...
  public async login() {
    ...
    this.auth.post('api/auth/login', params).then((res: any) => {
      var json = res;
      if (res.resultCode === 200) {
        let data = res.row;
        ...
        if (res.resultCode == 503) {
          ...
          this.auth.presentAlert(
            this.auth.message.get('login.invalidPassword')
          );
        }
      }
    });
      ...
      (res: any) => {
        ...
        if (res.resultCode === 200) {
          ...
          if (
            data == null ||
            data.device == null ||
            data.device.installedApp == null ||
            this.auth.isEmpty(data.device.installedApp.pushKey) ||
            this.auth.pushToken != data.device.installedApp.pushKey
          ) {
          ...
          this.auth.promptAlert(this.auth.message.get('login.alamPopup')).then(
            (res) => this.regDevice(user, true),
            (err) => this.regDevice(user, false)
          );
        }
      },
      (error: any) => {
  ...
  private async regDevice(user: any, pushActivate: boolean) {
    ...
    console.log('deviceId : ' + this.auth.getDeviceId());
    let installedApp = {};
    ...
    let device = {};
    device['deviceId'] = this.auth.getDeviceId();
    ...
    this.auth.post2('api/auth/registDevice', params).then((res: any) => {
      if (res.resultCode === 200) {
        let data = res.row;
        user.device = data.device;
        user.talkDenyDay = data.talkDenyDay;
  ...
  private async loginCheck(user: any) {
    if (user.certificationLevel != null && user.certificationLevel!! < 11) {
      this.auth
        .promptAlert(this.auth.message.get('login.verificationPopup'))
        .then((res) => {
          this.auth
            .setStorage(common.VERIFICATION_KEY, common.LEVEL_UP)
            .then(() => {
      ...
      if (user.useStatus == 'waitingToLeave') {
        ...
        this.auth
          .promptAlert(this.auth.message.get('login.waitingToLeavePopup'))
          .then((res) => {
            this.auth
              .setStorage(common.VERIFICATION_KEY, common.CANCEL_LEAVE)
              .then(() => {
                this.auth.navigateForward('/signup2');
              });
          });
  ...
  private getNotSignedTermsList() {
    ...
    this.auth
      .post('api/auth/getNotSignedActiveTermsAll', params)
      .then((res: any) => {
        if (res.resultCode === 200) {
